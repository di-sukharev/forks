	...
	constructor() {
		super({
			...
			menu: [
				...
				{
					id: MenuId.MenubarApforksanceMenu,
...
class FocusforksAIExtensionAction extends Action2 {
	static readonly ID = 'workbench.action.focusforksAIExtension';
	static readonly LABEL = localize2(
		"focusforksAIExtension",
		"Focus into forksAI Extension",
	);

	constructor() {
		super({
			id: FocusforksAIExtensionAction.ID,
			title: FocusforksAIExtensionAction.LABEL,
			category: Categories.View,
			f1: true,
			// keybinding: do not add keybinding CTRL/CMD L here, it comes from forksai extension
		});
	}

	override async run(accessor: ServicesAccessor): Promise<void> {
		// focus forksai extension
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand('forksai.focusContinueInput');
	}
}

registerAction2(FocusforksAIExtensionAction);
...
// Following is a only forksAI related action, need to refactor these type of actions to separate file
import { IOpenerService } from 'vs/platform/opener/common/opener';
import { URI } from 'vs/base/common/uri';
import { IProductService } from 'vs/platform/product/common/productService';

class OpenforksAIDocsAction extends Action2 {
	static readonly ID = 'workbench.action.openforksAIDocs';
	static readonly LABEL = localize2(
		"openforksAIDocs",
		"Open forksAI Documentation",
	);

	constructor() {
		super({
			id: OpenforksAIDocsAction.ID,
			title: OpenforksAIDocsAction.LABEL,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (!productService.forksAILinks?.docs) {
			return;
		}
		await openerService.open(URI.parse(productService.forksAILinks?.docs));
	}
}

registerAction2(OpenforksAIDocsAction);
