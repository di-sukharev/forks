...
import { AllEditorsByMostRecentlyUsedQuickAccess, ActiveGroupEditorsByMostRecentlyUsedQuickAccess, AllEditorsByApforksanceQuickAccess } from 'vs/workbench/browser/parts/editor/editorQuickAccess';
...
abstract class AbstractCloseAllAction extends Action2 {
	...
	protected groupsToClose(editorGroupService: IEditorGroupsService): IEditorGroup[] {
		...
		// Close editors in reverse order of their grid apforksance so that the editor
		...
		const groups = editorGroupService.getGroups(GroupsOrder.GRID_APforksANCE);
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		for (const { editor, groupId } of editorService.getEditors(EditorsOrder.SEQUENTIAL, { excludeSticky: this.excludeSticky })) {
			...
			// dialog apforkss, so just track that separate
			...
			// when a native dialog apforkss, so just track that separate
...
export class ShowAllEditorsByApforksanceAction extends Action2 {

	static readonly ID = 'workbench.action.showAllEditors';

	constructor() {
		super({
			id: ShowAllEditorsByApforksanceAction.ID,
			title: localize2('showAllEditors', 'Show All Editors By Apforksance'),
			f1: true,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		quickInputService.quickAccess.show(AllEditorsByApforksanceQuickAccess.PREFIX);
	}
}
